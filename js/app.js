(() => {
    ...
    function spollers() {
        ...
        document.addEventListener("click", (function(e) {
            ...
            if (isMobile) {
                ...
                const beforeWidth = 25;
                const clickX = e.clientX - rect.left;
                const clickY = e.clientY - rect.top;
                if (clickX >= 0 && clickX <= beforeWidth && clickY >= 0 && clickY <= rect.height) {
                    const spollersBlock = details.closest("[data-spollers]");
                    const spollerSpeed = getSpollerSpeed(spollersBlock);
                    summary.classList.toggle("_spoller-active");
                    _slideToggle(summary.nextElementSibling, spollerSpeed);
                    if (details.parentNode?.dataset.oneSpoller) hideSpollersBody(details.parentNode);
                }
            }
        }));
            ...
            spollersArray.forEach((spollersBlock => {
                ...
                if (matchMedia.matches || !matchMedia) {
                    ...
                    if (window.innerWidth >= 992) {
                        const hoverSpoilers = block.querySelectorAll("details.spoller-hover");
                        hoverSpoilers.forEach((spoller => {
                            const summary = spoller.querySelector("summary");
                            ...
                            summary.addEventListener("mouseenter", (() => {
                                ...
                                if (!spoller.open) {
                                    spoller.open = true;
                                    summary.classList.add("_spoller-active");
                            ...
                            summary.addEventListener("mouseleave", (() => {
                                leaveTimeout = setTimeout((() => {
                                    spoller.open = false;
                                    summary.classList.remove("_spoller-active");
                                    content.hidden = true;
                                }), 150);
                            }));
            ...
            spollerItems.forEach((item => {
                ...
                if (hideSpollerBody) {
                    ...
                    if (!item.hasAttribute("data-open")) {
                        item.open = false;
                        summary.nextElementSibling.hidden = true;
                    } else {
                        summary.classList.add("_spoller-active");
                        item.open = true;
                    }
                    ...
                    summary.classList.remove("_spoller-active");
                    item.open = true;
                    summary.nextElementSibling.hidden = false;
                }
            }));
        }
        function setSpollerAction(e) {
            ...
            if (!mobile && el.closest("details.spoller-hover")) {
                if (el.closest("a")) return;
                e.preventDefault();
                return;
            }
            if (mobile && el.closest("summary") && el.closest("a")) return;
            const summary = el.closest("summary");
            if (summary && summary.closest("[data-spollers]")) {
                const isLinkClick = el.closest("a");
                if (isLinkClick) return;
                e.preventDefault();
                const spollerBlock = summary.closest("details");
                const spollersBlock = summary.closest("[data-spollers]");
                if (!spollersBlock.classList.contains("_spoller-init")) return;
                if (spollersBlock.querySelectorAll("._slide").length) return;
                ...
                summary.classList.toggle("_spoller-active");
                _slideToggle(summary.nextElementSibling, spollerSpeed);
                if (scrollSpoller && summary.classList.contains("_spoller-active")) {
                    const offset = +spollerBlock.dataset.spollerScroll || 0;
                    const noHeader = spollerBlock.hasAttribute("data-spoller-scroll-noheader") ? document.querySelector(".header")?.offsetHeight || 0 : 0;
                    window.scrollTo({
                        top: spollerBlock.offsetTop - (offset + noHeader),
                        behavior: "smooth"
                    });
                }
            }
        ...
        function hideSpollersBody(spollersBlock) {
            const active = spollersBlock.querySelector("details[open]");
            if (active && !spollersBlock.querySelectorAll("._slide").length) {
                const summary = active.querySelector("summary");
                const spollerSpeed = getSpollerSpeed(spollersBlock);
                summary.classList.remove("_spoller-active");
                _slideUp(summary.nextElementSibling, spollerSpeed);
                setTimeout((() => {
                    active.open = false;
                }), spollerSpeed);
            }
        }
        function getSpollerSpeed(block) {
            return block?.dataset?.spollersSpeed ? parseInt(block.dataset.spollersSpeed) : 500;
        }
    }
    function tabs() {
    ...
    function formRating() {
        ...
        function formRatingSet(rating, value) {
            ...
            if (resultFullItems > 0) rating.querySelector(".rating__items").classList.add("rating--last-active");
            ratingItems.forEach(((ratingItem, index) => {
                ratingItem.classList.remove("rating__item--active");
                ratingItem.classList.remove("rating__item--partial");
                ratingItem.querySelector("span") ? ratingItem.querySelector("span").remove() : null;
                if (index < resultFullItems) {
                    ratingItem.classList.add("rating__item--active");
                    ratingItem.style.display = "inline-block";
                } else if (index === resultFullItems && resultPartItem > 0) {
                    ratingItem.classList.add("rating__item--partial");
                    ratingItem.style.display = "inline-block";
                    ratingItem.insertAdjacentHTML("beforeend", `<span style="width:${resultPartItem * 100}%"></span>`);
                } else ratingItem.style.display = "none";
            }));
        }
    }
    ...
    rangeInit();
    function range2Init() {
        const priceSlider2 = document.querySelector("#range2");
        if (priceSlider2) {
            priceSlider2.getAttribute("data-from");
            priceSlider2.getAttribute("data-to");
            initialize(priceSlider2, {
                start: [ 50, 200 ],
                connect: true,
                range: {
                    min: 0,
            ...
            mergeTooltips2(priceSlider2, 15, " - ");
            function mergeTooltips2(slider, threshold, separator) {
                var textIsRtl = getComputedStyle(slider).direction === "rtl";
                var isRtl = slider.noUiSlider.options.direction === "rtl";
                var isVertical = slider.noUiSlider.options.orientation === "vertical";
                var tooltips = slider.noUiSlider.getTooltips();
    ...
    function initSliders() {
        if (document.querySelector(".customers__slider")) new swiper_core_Swiper(".customers__slider", {
            ...
            loop: true,
            navigation: {
                prevEl: ".customers__slide-button-prev",
                nextEl: ".customers__slide-button-next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1,
        ...
        if (document.querySelector(".main-product__thumbs-slider") && document.querySelector(".main-product__slider")) {
            let thumbsSlider;
            const initSwiper = () => {
                const isSmallScreen = window.innerWidth < 992;
                if (thumbsSlider && thumbsSlider.destroy) thumbsSlider.destroy(true, true);
                thumbsSlider = new swiper_core_Swiper(".main-product__thumbs-slider", {
                ...
                new swiper_core_Swiper(".main-product__slider", {
                    modules: [ Thumb ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 1,
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        const closeButton = document.querySelector(".upper-header__close-button");
        const upperHeader = document.querySelector(".upper-header");
        if (closeButton && upperHeader) closeButton.addEventListener("click", (function() {
            upperHeader.style.display = "none";
        }));
    }));
    document.addEventListener("DOMContentLoaded", (function() {
        const passwordInput = document.getElementById("password-sign-up");
        const confirmPasswordInput = document.getElementById("confirm-password-sign-up");
        const togglePasswordButton = document.querySelector(".sign-up__show-password-button");
        const toggleConfirmPasswordButton = document.querySelector(".sign-up__show-confirm-password-button");
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        ...
        if (!searchForm || !searchButton || !logo) return;
        searchButton.addEventListener("click", (e => {
            if (window.innerWidth <= 495) if (!searchForm.classList.contains("visible")) {
        ...
        document.addEventListener("click", (e => {
            if (window.innerWidth <= 495 && searchForm.classList.contains("visible")) {
    ...
    function adjustCounterBorders() {
        ...
        counters.forEach((counter => counter.classList.remove("wrapped-last")));
        if (counters.length >= 3) {
            const second = counters[1];
            const third = counters[2];
            if (second.offsetTop !== third.offsetTop) second.classList.add("wrapped-last");
        }
    }
    window.addEventListener("resize", adjustCounterBorders);
    document.addEventListener("DOMContentLoaded", adjustCounterBorders);
    window["FLS"] = true;
    addTouchClass();
    addLoadedClass();
    menuInit();
